  ...
  const renderAssistantJobCards = (message: ChatMessage) => {
    ...
    return (
      <div className="space-y-2.5">
        ...
        <div className="flex flex-wrap gap-2 rounded-2xl border border-white/10 bg-[#1a1a1a] px-3 py-3">
          {remainingJobs > 0 ? (
            <button
              type="button"
              onClick={() => showMoreJobsForMessage(message)}
              className="rounded-full bg-white/8 px-3 py-2 text-xs font-semibold text-white shadow-sm hover:bg-white/12"
            >
              More jobs ({remainingJobs})
            </button>
          ) : (
            <span className="rounded-full bg-white/8 px-3 py-2 text-xs font-semibold text-white/60 shadow-sm">
              All jobs shown
            </span>
          )}
      ...
      <aside className="hidden h-[100dvh] max-h-[100dvh] min-h-0 w-[260px] min-w-[260px] shrink-0 overflow-hidden border-r border-white/10 bg-[radial-gradient(circle_at_top,_rgba(255,255,255,0.08),_transparent_28%),_#0b0b0b] text-white lg:flex lg:flex-col">
        ...
        <div className="fm-desktop-sidebar-scroll min-h-0 flex-1 overflow-y-auto overflow-x-hidden px-3 py-3">
          ...
          <button
            ...
            className="mb-1 flex w-full min-w-0 items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm text-slate-600 transition hover:bg-slate-100 hover:text-slate-900"
          >
