...
function CompactAssistantJobCard({
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-[#111111] p-3 shadow-[0_14px_35px_rgba(0,0,0,0.22)]">
      <div className="min-w-0">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-0.5 line-clamp-1 text-xs font-medium text-white/60">
              {job.company}
            </p>
          </div>
        ...
        <div className="mt-2 flex flex-wrap gap-1.5">
          {job.category && (
            <span className="rounded-full bg-white/8 px-2 py-0.5 text-[11px] font-medium text-white/75">
              {job.category}
            </span>
          )}
        ...
        <div className="mt-3 grid grid-cols-3 gap-2">
          <Button
            ...
            onClick={onOpen}
            disabled={disabled}
            className="h-9 rounded-xl border-white/10 bg-[#1a1a1a] text-xs text-white hover:bg-white/10"
          >
            <ExternalLink className="mr-1.5 h-3.5 w-3.5" />
            {deadlineInfo.expired ? 'Closed' : 'Open'}
          </Button>

          <Button size="sm" variant="outline" onClick={onVerify} className="h-9 rounded-xl border-white/10 bg-[#1a1a1a] text-xs text-white hover:bg-white/10">
            <ShieldCheck className="mr-1.5 h-3.5 w-3.5" />
            Verify
          </Button>

          <Button size="sm" variant="outline" onClick={onSave} className="h-9 rounded-xl border-white/10 bg-[#1a1a1a] text-xs text-white hover:bg-white/10">
        ...
        <button
          ...
          className="mt-2 flex w-full items-center justify-between rounded-xl px-1 py-1 text-[12px] font-semibold text-white/60 transition hover:text-white"
        >
        ...
        {detailsOpen && (
          <div className="mt-2 rounded-xl bg-white/5 p-3 text-[12px] leading-relaxed text-white/70">
            <p>
            ...
            <p>
              <span className="font-semibold text-white">Closing date:</span>{' '}
              {getClosingDateDisplay(job.deadline)}
            </p>

            {job.salary && (
              <p>
                <span className="font-semibold text-white">Salary:</span> {job.salary}
              </p>
            )}

            {job.description && (
              <p>
                <span className="font-semibold text-white">Details:</span> {job.description}
              </p>
            )}

            <p>
              <span className="font-semibold text-white">Source:</span> {job.sourceLabel}
            </p>
          </div>
        )}
      </div>
...
const [modeMenuOpen, setModeMenuOpen] = useState(false);
  ...
  const handleAddSubfolder = () => {
    ...
    setSelectedProjectFolder(trimmedFolderName);
    setNewSubfolderName('');
    setProjectsOpen(false);
    toast({
      title: 'Subfolder added',
      description: `${trimmedFolderName} is active and ready for project chat.`,
    });
  };
  ...
  const submitGuidedSearch = () => {
    const nextWorkType = guidedSearchJob.trim();
    const nextLocation = guidedSearchLocation.trim();

    if (!nextWorkType || !nextLocation) return;

    setGuidedSearchOpen(false);
    quickAsk(`Find ${nextWorkType} jobs in ${nextLocation}. I want opportunities in this area, with employer details, salary if available, requirements, closing dates, and official application links. Please search for the best matches for my chosen work type and area.`);
  };
  ...
  const sendPrompt = async (overridePrompt?: string): Promise<string | undefined> => {
    ...
    const projectPromptPrefix = activeProject && selectedProjectFolder
      ? `Project context: ${activeProject.name} / ${selectedProjectFolder}. Work inside this project folder and keep the answer organised for this project.`
      : '';

    const aiPromptWithProjectContext = projectPromptPrefix
      ? `${projectPromptPrefix}\n\n${finalPrompt}`
      : finalPrompt;
    ...
    const aiPromptWithToolDirection = addFaceMeXCareerToolInstruction(aiPromptWithProjectContext, intent);
...
Apply link: ${job.applyUrl}`;
  ...
  const renderAssistantJobCards = (message: ChatMessage) => {
    ...
    return (
      <div className="space-y-2.5">
        <div className="sticky top-0 z-10 rounded-2xl border border-white/10 bg-[#111111]/95 px-3 py-2 shadow-sm backdrop-blur lg:border-white/10 lg:bg-[#111111]">
          <div className="flex items-center justify-between gap-2">
            <div className="flex min-w-0 items-center gap-2">
              <Briefcase className="h-4 w-4 shrink-0 text-white/60" />
              <p className="truncate text-[13px] font-semibold text-white">
                {totalLabel}
              </p>
            </div>

            {busy && <Loader2 className="h-4 w-4 animate-spin text-white/60" />}
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 rounded-2xl border border-white/10 bg-[#121212] px-3 py-3">
          {remainingJobs > 0 ? (
            <button
              type="button"
              onClick={() => showMoreJobsForMessage(message)}
              className="rounded-full border border-white/10 bg-[#1a1a1a] px-3 py-2 text-xs font-semibold text-white shadow-sm hover:bg-white/10"
            >
              More jobs ({remainingJobs})
            </button>
          ) : (
            <span className="rounded-full border border-white/10 bg-[#1a1a1a] px-3 py-2 text-xs font-semibold text-white/60 shadow-sm">
              All jobs shown
            </span>
          )}
  ...
  const assistantSmartActions = (message: ChatMessage, previousUserText = '') => {
    ...
    if (isGovernment) {
      return (
        <div className="mt-3 grid grid-cols-2 gap-2 border-t border-black/5 pt-3 dark:border-white/10">
          ...
          >
            <Button size="sm" variant="outline" className="h-9 w-full rounded-xl border border-white/10 bg-[#171717] text-xs text-white hover:bg-white/10 hover:text-white">
          ...
          <Button
            ...
            className="h-9 rounded-xl border border-white/10 bg-[#171717] text-xs text-white hover:bg-white/10 hover:text-white"
          >
    ...
    if (showCvButtons) {
      return (
        <div className="mt-3 grid grid-cols-2 gap-2 border-t border-black/5 pt-3 dark:border-white/10">
          <Button size="sm" variant="outline" onClick={openCvBuilder} className="h-10 rounded-xl border border-white/10 bg-[#171717] text-xs text-white hover:bg-white/10 hover:text-white">
            <FileText className="mr-2 h-3.5 w-3.5" />
            AI CV Builder
          </Button>

          <Button size="sm" variant="outline" onClick={() => copyText(message.content)} className="h-10 rounded-xl border border-white/10 bg-[#171717] text-xs text-white hover:bg-white/10 hover:text-white">
            <Copy className="mr-2 h-3.5 w-3.5" />
            Copy Answer
          </Button>

          <Button size="sm" variant="outline" onClick={() => saveMessageAs(message.id, 'cv_advice')} className="h-10 rounded-xl border border-white/10 bg-[#171717] text-xs text-white hover:bg-white/10 hover:text-white">
      ...
      return (
        <div className="mt-3 grid grid-cols-2 gap-2 border-t border-black/5 pt-3 dark:border-white/10 lg:border-white/10">
          <Button
            ...
            className="h-10 rounded-xl border border-white/10 bg-[#171717] text-xs text-white hover:bg-white/10 hover:text-white"
          >
  ...
  return (
    <div className="fm-ai-workspace fixed inset-0 z-50 flex h-[100dvh] max-h-[100dvh] w-screen max-w-full min-w-0 overflow-hidden bg-[#0d0d0d] text-white lg:bg-[#0d0d0d] lg:text-white">
      <style>{`
        ...
        @media (min-width: 1024px) {
          .fm-user-prompt-bubble,
          .dark .fm-user-prompt-bubble {
            background: linear-gradient(180deg, rgba(32, 32, 32, 0.96), rgba(24, 24, 24, 0.98));
            color: #ffffff;
        ...
        @media (max-width: 1023px) {
          ...
          .fm-ai-workspace .fm-assistant-message button,
          .fm-ai-workspace .fm-assistant-message [role='button'],
          .fm-ai-workspace .fm-assistant-message .rounded-xl,
          .fm-ai-workspace .fm-assistant-message .rounded-2xl,
          .fm-ai-workspace .fm-assistant-message .rounded-full {
            background: #171717 !important;
      ...
      <aside className="hidden h-[100dvh] max-h-[100dvh] min-h-0 w-[260px] min-w-[260px] shrink-0 overflow-hidden border-r border-white/10 bg-[radial-gradient(circle_at_top,_rgba(255,255,255,0.08),_transparent_28%),_#0b0b0b] text-white lg:flex lg:flex-col">
        <div className="flex h-16 shrink-0 items-center justify-between border-b border-white/10 bg-[#0b0b0b] px-4">
          <button
            type="button"
            onClick={openNewChatCard}
            className="flex h-9 items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 text-sm font-medium text-white transition hover:bg-white/10 hover:text-white"
          >
            <Edit3 className="h-4 w-4" />
            New chat
          </button>
        ...
        <div className="fm-desktop-sidebar-scroll min-h-0 flex-1 overflow-y-auto overflow-x-hidden px-3 py-3">
          ...
          >
            ...
            <kbd className="rounded border border-white/10 bg-white/5 px-1.5 py-0.5 text-[10px] font-medium text-white/60">Ctrl K</kbd>
          </button>
            ...
            <div className="mb-4 rounded-2xl border border-white/10 bg-[#0b0b0b] p-3 text-white">
              <div className="flex items-center gap-2 rounded-full border border-white/10 bg-[#111] px-3 py-2">
                ...
                <button
                  ...
                  className="rounded-full border border-white/10 bg-[#171717] px-3 py-1.5 text-[12px] font-semibold text-white transition hover:bg-white/10 active:scale-[0.98]"
                >
                  Search
                </button>
              </div>
      ...
      {!(globalSearchOpen || jobsOpen || watchPanelOpen || subscriptionOpen || scheduleOpen || applySheetOpen || clearWorkspaceOpen || trackerOpen || libraryOpen || Boolean(savedReaderMessage)) && (
        <div className="fixed right-4 top-4 z-[75] hidden items-center gap-2 lg:flex">
          <button
            type="button"
            onClick={() => setFocusMode((value) => !value)}
            className="h-8 items-center gap-1 whitespace-nowrap rounded-full border border-white/10 bg-white/8 px-2 py-1 text-[11px] font-semibold tracking-[0.14em] text-white/90 transition hover:bg-white/12 lg:flex"
          ...
          <button
            ...
            className="h-8 items-center gap-1 whitespace-nowrap rounded-full border border-white/10 bg-white/10 px-2 py-1 text-[11px] font-semibold tracking-[0.14em] text-white/85 transition hover:bg-white/15 lg:flex"
          >
        ...
        <section className="mx-auto flex h-full w-full max-w-[760px] flex-col overflow-hidden rounded-none border-0 bg-[#0d0d0d] text-white shadow-none lg:max-w-none lg:rounded-[30px] lg:border lg:border-white/8 lg:bg-[#111111] lg:text-white lg:shadow-[0_18px_60px_rgba(0,0,0,0.42)]">
          ...
          <footer className="shrink-0 border-t border-white/5 bg-[#0d0d0d]/95 p-2 pb-[calc(env(safe-area-inset-bottom)+0.5rem)] backdrop-blur-xl dark:border-white/5 dark:bg-[#111]/95 sm:p-4 lg:border-white/10 lg:bg-[#111111] lg:px-6 lg:pb-6">
            <div className="mx-auto w-full max-w-3xl lg:max-w-[760px]">
              ...
              <div className="fm-composer-card rounded-full border border-white/10 bg-[#1a1a1a] px-2 py-2 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] sm:px-3 lg:border-white/10 lg:bg-[#171717]">
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => imageInputRef.current?.click()}
                    className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#2b2b2b] text-white/80 transition active:scale-[0.98] hover:bg-white/10"
                    aria-label="Upload image or document"
                  >
                    <Plus className="h-4 w-4" />
                  </button>

                  <div className="flex min-h-[40px] flex-1 items-center rounded-full border border-white/10 bg-[#111111] px-3 py-1.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
                    <Textarea
                      value={prompt}
                      onChange={(e) => setPrompt(e.target.value)}
                      onFocus={() => setFollowUpExpanded(true)}
                      placeholder={activeProject && selectedProjectFolder ? `${activeProject.name} / ${selectedProjectFolder}` : composerPlaceholder}
                      className={`min-h-[24px] flex-1 min-w-0 max-w-full resize-none border-0 bg-transparent px-1 py-1 text-[15px] leading-5 text-white/90 placeholder:text-white/45 shadow-none outline-none focus-visible:ring-0 focus-visible:ring-offset-0 ${
                        inputHasContent ? 'max-h-28' : 'h-7 max-h-8 overflow-hidden'
                      }`}
                      onKeyDown={(e) => {
                  ...
                  <Button
                    ...
                    className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full shadow-lg shadow-black/10 transition ${
                      inputHasContent
                        ? 'bg-black text-white hover:bg-[#111111] disabled:bg-black disabled:text-white'
                        : 'bg-[#2b2b2b] text-white/40 disabled:bg-[#2b2b2b] disabled:text-white/40'
                    }`}
                    aria-label="Send"
                  >
            ...
            {guidedSearchStep === 'job' ? (
              <div className="space-y-4">
                <input
                  ...
                  placeholder="Industry or work type: IT, admin, retail, finance..."
              ...
              <div className="space-y-4">
                <input
                  ...
                  placeholder="Area you want to work: Tzaneen, Johannesburg, Cape Town..."
                  className="h-12 w-full rounded-2xl border border-white/10 bg-[#171717] px-3 text-[15px] text-white outline-none placeholder:text-white/45"
                  autoFocus
                />
      ...
      {jobsOpen && (
        <div className="fixed inset-0 z-[70] bg-black/60 backdrop-blur-[4px] lg:hidden" onClick={() => setJobsOpen(false)}>
          <div
            className="fm-premium-drawer absolute left-0 top-0 flex h-full w-[88vw] max-w-[390px] flex-col overflow-hidden border-r border-white/10 bg-[#171717] text-white shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <div className="fm-mobile-sidebar-scroll min-h-0 flex-1 overflow-y-auto overflow-x-hidden px-3 py-3">
              <button
                ...
                className="mb-3 flex w-full items-center gap-3 rounded-lg border border-white/10 bg-[#212121] px-3 py-2.5 text-left text-sm font-medium text-white transition hover:bg-[#2a2a2a]"
              >
              ...
              >
                <Globe2 className="h-4 w-4" />
                <span className="min-w-0 flex-1">Watch lessons</span>
              ...
              <button
                ...
                onClick={() => {
                  setJobsOpen(false);
                  quickAsk('Help me prepare for an interview. Give me questions and strong answers.');
                }}
                className="mb-1 flex w-full min-w-0 items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm text-white/75 transition hover:bg-white/10 hover:text-white"
              >
                <Users className="h-4 w-4" />
              ...
              {chatSessions.length > 0 && (
                <>
                  <div className="mt-7 px-3 text-[11px] font-semibold uppercase tracking-[0.12em] text-white/35">Recent</div>
                  ...
                  <div className="mt-2 space-y-1">
                    {chatSessions.slice(0, 12).map((session) => (
                      ...
                      >
                        <span className="block max-w-full whitespace-normal break-words leading-5">{session.title || 'New chat'}</span>
                      </button>
                    ))}
                  </div>
                </>
          ...
          <div
            ...
            onTouchEnd={(event) => {
              ...
              if (diff < -72) {
                ...
                setTimeout(() => {
                  const input = document.getElementById('new-project-input') as HTMLInputElement | null;
                  input?.focus();
                }, 80);
              }
            }}
          >
            ...
            {projectComposerOpen && (
              <div className="px-4 pb-3">
                <div className="flex gap-2 rounded-2xl border border-white/10 bg-[#101010] p-2">
                  <input
                    id="new-project-input"
                    value={newProjectName}
                    onChange={(event) => setNewProjectName(event.target.value)}
                    placeholder="Project name"
                    className="min-w-0 flex-1 bg-transparent px-2 py-2 text-[15px] text-white outline-none placeholder:text-[#a1a1aa]"
                    onKeyDown={(event) => {
                      if (event.key === 'Enter') {
                        handleCreateProject();
                      }
                  ...
                  <button
                    ...
                    className="rounded-full bg-blue-600 px-3 py-2 text-[12px] font-semibold text-white"
                  >
                    Create
                  </button>
                </div>
                    ...
                    return (
                      <div
                        ...
                        className={`rounded-2xl border p-2 transition ${
                          isSelected ? 'border-white/15 bg-[#111111]' : 'border-transparent bg-transparent'
                        }`}
                      >
                        <button
                          type="button"
                          onClick={() => {
                            ...
                            setSelectedProjectFolder(project.folders?.[0] || 'General');
                          }}
                          className="flex w-full items-center gap-3 rounded-xl p-3 text-left transition hover:bg-[#171717]"
                        >
                        ...
                        {isSelected && (
                          <div className="mt-2 space-y-2 px-2 pb-1">
                            {project.folders.map((folder) => (
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  setProjectsOpen(false);
                                  toast({
                                    title: 'Project folder active',
                                    description: `${project.name} / ${folder} is ready for project chat.`,
                                  });
                                }}
                                className={`flex w-full items-center justify-between rounded-xl border px-3 py-2 text-left text-[13px] transition ${
                                  selectedProjectFolder === folder
                                    ...
                                    : 'border-white/5 bg-[#0b0b0b] text-white/70 hover:bg-[#171717]'
                                }`}
                              >
                            ...
                            <div className="mt-2 flex gap-2 rounded-xl border border-dashed border-white/10 bg-[#0b0b0b] p-2">
                              <input
                                value={newSubfolderName}
                                onChange={(event) => setNewSubfolderName(event.target.value)}
                                placeholder="New folder name"
                                className="min-w-0 flex-1 bg-transparent px-2 py-2 text-[13px] text-white outline-none placeholder:text-[#a1a1aa]"
                              />
                              <button
                                type="button"
                                onClick={handleAddSubfolder}
                                className="rounded-full bg-white px-3 py-2 text-[11px] font-semibold text-black"
                              >
                                Add
                              </button>
                            </div>
            ...
            <div className="px-4 pb-3 text-[11px] text-[#a1a1aa]">
              ...
              <span className="text-[#f4f4f5]">left to create</span>
              <span className="mx-2 text-[#a1a1aa]">•</span>
              <span className="text-[#f4f4f5]">right to close</span>
            </div>
          </div>
        </div>
      )}
