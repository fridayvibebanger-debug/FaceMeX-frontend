...
import { AlignCenter, AlignLeft, AlignRight, Bold, Check, Clipboard, Download, FileText, Highlighter, Italic, Link, List, Menu, Minus, Plus, Printer, Redo2, Save, Sparkles, Strikethrough, Underline, Undo2, Wand2, X } from 'lucide-react';
      ...
      <SensitiveContentShield context={kind === 'cv' ? 'cv' : 'cover-letter'} className="mx-auto max-w-[1440px] px-3 pb-8 pt-4 sm:px-6 lg:px-10 lg:pb-12 lg:pt-8">
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(300px,380px)_1fr]">
          ...
          <section className="relative min-w-0 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm sm:p-5 lg:border-slate-700 lg:bg-[#181d24] lg:p-6 lg:shadow-[0_20px_60px_rgba(0,0,0,.2)]">
            ...
            <div className="mb-4 flex items-center justify-between rounded-t-xl border border-slate-700 bg-[#080a0d] px-3 py-2 text-white shadow-lg">
              <div className="flex items-center gap-2"><span className="text-xs font-bold tracking-wide">Home</span><span className="text-[11px] text-slate-400">Document editor</span></div>
              <Button type="button" variant="ghost" size="sm" onClick={() => setTemplatesOpen((value) => !value)} className="h-9 shrink-0 gap-1.5 rounded-md border border-white/10 px-3 text-xs text-slate-200 hover:bg-white/10 hover:text-white sm:h-8"><Menu className="h-4 w-4" />{templatesOpen ? <><span className="hidden sm:inline">Hide templates</span><span className="sm:hidden">Hide</span></> : <><span className="hidden sm:inline">Show templates</span><span className="sm:hidden">Templates</span></>}</Button>
            </div>
            <div className="mb-4 flex max-w-full flex-nowrap items-center gap-1.5 overflow-x-auto rounded-b-xl border border-t-0 border-slate-700 bg-[#11151b] p-2 shadow-lg">
              <span className="shrink-0 px-1 text-[10px] font-semibold uppercase tracking-wider text-slate-500">Clipboard</span>
              <Button type="button" variant="outline" size="icon" onClick={copyDocument} className="h-9 w-9 shrink-0 rounded-md border-slate-600 bg-[#181d24] text-slate-200 hover:bg-[#252c36] hover:text-white" aria-label="Copy document"><Clipboard className="h-4 w-4" /></Button>
              <Button type="button" variant="outline" size="icon" onClick={selectAll} className="h-9 w-9 shrink-0 rounded-md border-slate-600 bg-[#181d24] text-slate-200 hover:bg-[#252c36] hover:text-white" aria-label="Select all text"><Check className="h-4 w-4" /></Button>
              ...
              <Button type="button" variant={bold ? 'default' : 'outline'} size="icon" onClick={() => { setBold((value) => !value); formatSelection('**'); }} className="h-9 w-9 shrink-0 rounded-md border-slate-600 bg-[#181d24] text-slate-200 hover:bg-[#252c36] hover:text-white" aria-label="Bold"><Bold className="h-4 w-4" /></Button>
              <Button type="button" variant={italic ? 'default' : 'outline'} size="icon" onClick={() => { setItalic((value) => !value); formatSelection('__'); }} className="h-9 w-9 shrink-0 rounded-md border-slate-600 bg-[#181d24] text-slate-200 hover:bg-[#252c36] hover:text-white" aria-label="Italic"><Italic className="h-4 w-4" /></Button>
              <Button type="button" variant={underline ? 'default' : 'outline'} size="icon" onClick={() => { setUnderline((value) => !value); formatSelection('~~'); }} className="h-9 w-9 shrink-0 rounded-md border-slate-600 bg-[#181d24] text-slate-200 hover:bg-[#252c36] hover:text-white" aria-label="Underline"><Underline className="h-4 w-4" /></Button>
              <Button type="button" variant="outline" size="icon" onClick={() => formatSelection('--')} className="h-9 w-9 shrink-0 rounded-md border-slate-600 bg-[#181d24] text-slate-200 hover:bg-[#252c36] hover:text-white" aria-label="Strikethrough"><Strikethrough className="h-4 w-4" /></Button>
              <Button type="button" variant="outline" size="icon" onClick={() => formatSelection('==')} className="h-9 w-9 shrink-0 rounded-md border-slate-600 bg-[#181d24] text-slate-200 hover:bg-[#252c36] hover:text-white" aria-label="Highlight text"><Highlighter className="h-4 w-4" /></Button>
              <span className="mx-1 h-6 w-px shrink-0 bg-slate-700" />
            ...
            {templatesOpen && <aside className="mb-4 max-h-[52vh] overflow-y-auto rounded-xl border border-slate-700 bg-[#11151b] p-3 shadow-lg lg:absolute lg:left-0 lg:top-0 lg:z-10 lg:mb-0 lg:h-full lg:max-h-none lg:w-60 lg:rounded-r-xl lg:rounded-l-none"><div className="mb-3 flex items-center justify-between"><div><p className="text-[10px] font-semibold uppercase tracking-[.18em] text-blue-400">Design library</p><h3 className="mt-1 text-sm font-semibold text-white">CV templates</h3></div><Button type="button" variant="ghost" size="icon" onClick={() => setTemplatesOpen(false)} className="h-9 w-9 shrink-0 rounded-md text-slate-400 hover:bg-white/10 hover:text-white" aria-label="Hide templates"><X className="h-4 w-4" /></Button></div><div className="grid grid-cols-2 gap-2 lg:grid-cols-1">{templates.map((item) => <button key={item.key} type="button" onClick={() => setTemplate(item.key)} className={`min-h-20 rounded-lg border p-3 text-left transition active:scale-[.98] ${template === item.key ? 'border-blue-500 bg-blue-500/15 ring-1 ring-blue-500' : 'border-slate-700 bg-[#181d24] hover:border-slate-500 hover:bg-[#202631]'}`}><span className="block text-sm font-semibold text-white">{item.label}</span><span className="mt-1 block text-[10px] leading-4 text-slate-400 sm:text-[11px]">{item.description}</span></button>)}</div></aside>}
