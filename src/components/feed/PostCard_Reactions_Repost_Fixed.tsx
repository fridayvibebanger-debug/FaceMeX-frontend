...
type ReactionType = 'love' | 'like' | 'haha' | 'wow' | 'smile' | 'sad' | 'angry';
...
const REACTION_OPTIONS: Array<{
  ...
  { type: 'like', label: 'Like', icon: ThumbsUp, className: 'text-blue-500' },
  { type: 'haha', label: 'Haha', icon: Laugh, className: 'text-amber-500' },
  { type: 'wow', label: 'Smile', icon: Smile, className: 'text-emerald-500' },
  { type: 'sad', label: 'Sad', icon: Frown, className: 'text-slate-500' },
  { type: 'angry', label: 'Angry', icon: Angry, className: 'text-red-500' },
];
...
function normalizePostDocuments(post: Post): PostDocumentItem[] {
  ...
  const addDoc = (raw: any, index: number) => {
    ...
    if (!url && pages.length === 0 && !previewUrl) return;
    ...
    const rawPreviewPages = Number(
      raw.previewPagesCount ||
        raw.preview_pages_count ||
        raw.unlockedPages ||
        raw.unlocked_pages ||
        raw.visiblePages ||
        raw.visible_pages ||
    ...
    docs.push({
      ...
      previewPages,
      previewUrl,
      mimeType: cleanString(raw.mimeType || raw.mime_type || raw.type),
    });
  };

  const rawDocuments =
    (post as any).documents ||
    (post as any).document_items ||
    (post as any).documentItems ||
    [];
  ...
  if (docs.length === 0) {
    ...
    const directPreviewUrl =
      (post as any).documentPreviewUrl ||
      (post as any).document_preview_url ||
      (post as any).documentCover ||
      (post as any).document_cover ||
      directPages[0] ||
      '';

    if (directDocumentUrl || directPages.length || directPreviewUrl) {
      addDoc(
        {
...
function normalizeReaction(value: unknown): ReactionType | undefined {
  const reaction = cleanString(value).toLowerCase();

  if (reaction === 'love') return 'love';
  if (reaction === 'like') return 'like';
  if (reaction === 'haha') return 'haha';
  if (reaction === 'wow') return 'wow';
  if (reaction === 'smile') return 'smile';
...
function getReactionMeta(reaction?: ReactionType) {
  const finalReaction = reaction || 'like';
  return (
    REACTION_OPTIONS.find((item) => item.type === finalReaction) ||
    REACTION_OPTIONS.find((item) => item.type === 'like') ||
    REACTION_OPTIONS[0]
  );
}
...
function renderLinkedText(text: string) {
  ...
  return parts.map((part, index) => {
    ...
    if (urlRegex.test(part)) {
      ...
      const { cleanUrl, trailing } = trimTrailingUrlPunctuation(part);
      const href = toClickableHref(cleanUrl);
      ...
      return (
        <span key={`${part}-${index}`}>
          <a
            ...
            className="break-all font-semibold text-blue-600 underline underline-offset-2 hover:text-blue-700 dark:text-blue-300"
...
function ReactionButtonIcon({ reaction, active }: { reaction?: ReactionType; active: boolean }) {
  if (!active) {
  ...
  return <Icon className={`h-[18px] w-[18px] ${meta.className}`} />;
}
...
function DocumentPreview({
  ...
  const firstPage = document.previewUrl || document.pages[0] || '';
  const imageCount = document.pages.filter(Boolean).length;
  const canOpenDoc = Boolean(document.url);
  ...
  return (
    <div className="w-full max-w-full min-w-0 overflow-hidden rounded-[22px] border border-border/70 bg-background shadow-sm">
      {firstPage ? (
        ...
        <div className="relative overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 px-4 py-5 text-white">
          ...
          <div className="relative flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 text-xs text-white/70">
                {document.totalPages} page{document.totalPages === 1 ? '' : 's'} · Preview {document.previewPages}
              </p>
              ...
              {canOpenDoc && (
                <button
                  ...
                  className="mt-3 inline-flex items-center gap-2 rounded-full bg-white px-3 py-1.5 text-xs font-bold text-slate-950"
                >
                  Open document
      ...
      <div className="flex items-center justify-between gap-3 px-3 py-2">
        ...
        {canOpenDoc && (
          <button
            type="button"
            onClick={openDocument}
            className="inline-flex shrink-0 items-center gap-1 rounded-full border px-3 py-1 text-[11px] font-semibold text-muted-foreground hover:bg-muted"
          >
            Open
...
export default function PostCard({ post, open, onOpenChange }: PostCardProps) {
  if (!post) {
    return (
      <CreatePostModal
        open={Boolean(open)}
  ...
  const isRepost = Boolean(
    (post as any).repostOf ||
      ...
      (post as any).originalPostId ||
      (post as any).original_post_id
  );

  const originalAuthorName =
    cleanString((post as any).originalAuthorName) ||
    cleanString((post as any).original_author_name) ||
    '';
  ...
  const activeReaction = normalizeReaction((post as any).reaction);
  const hasReacted = Boolean((post as any).isLiked || activeReaction);
  const activeReactionMeta = getReactionMeta(activeReaction);
  ...
  const handleReactionSelect = async (reaction: ReactionType) => {
    try {
      await likePost(post.id, reaction);
    } catch (error) {
      console.error('Reaction failed:', error);
      alert('Could not react. Please try again.');
    }
  };
  ...
  const requestCollaboration = async () => {
    ...
    try {
      await inviteCollaborator(post.id, myId, {
        code: myCode,
        name: myName,
        avatar: user?.avatar || '',
      });
  ...
  const inviteCollaboratorByCode = async () => {
    ...
    try {
      await inviteCollaborator(post.id, value);
    } catch (err) {
      console.error(err);
  ...
  const acceptCollaborationRequest = async (profile: CollaboratorProfile) => {
    ...
    try {
      await acceptCollabInvite(post.id, profile.id);
    } catch (err) {
      console.error(err);
  ...
  const rejectCollaborationRequest = async (profile: CollaboratorProfile) => {
    ...
    try {
      await rejectCollabInvite(post.id, profile.id);
    } catch (err) {
      console.error(err);
  ...
  const handleRepost = async () => {
    ...
    const ok = !reposted || window.confirm('You already reposted this. Repost again?');
    if (!ok) return;
    ...
    try {
      ...
      const repostText = cleanPostContent
        ? `Reposted from ${displayName}\n\n${cleanPostContent}`
        : `Reposted from ${displayName}`;
      ...
      await addPost(
        repostText,
        imageItems.length ? imageItems : undefined,
        (post as any).audio || undefined,
        undefined,
        ((post as any).mode === 'professional' ? 'professional' : 'social'),
        {
          repostOf: post.id,
          repost_of: post.id,
          ...
          original_author_name: displayName,
          ...
          documentUrl: documentItems[0]?.url || '',
          documentTitle: documentItems[0]?.title || '',
          documentPages: documentItems[0]?.pages || [],
          documentPreviewUrl: documentItems[0]?.previewUrl || '',
          documentTotalPages: documentItems[0]?.totalPages || 0,
          documentPreviewPages: documentItems[0]?.previewPages || 1,

          document_url: documentItems[0]?.url || '',
          document_title: documentItems[0]?.title || '',
          document_pages: documentItems[0]?.pages || [],
          document_preview_url: documentItems[0]?.previewUrl || '',
          document_total_pages: documentItems[0]?.totalPages || 0,
          document_preview_pages: documentItems[0]?.previewPages || 1,
        }
      );
    ...
    >
      <Card className="mb-3 w-full max-w-full min-w-0 overflow-hidden rounded-[24px] border bg-card shadow-[0_4px_14px_rgba(15,23,42,0.05)]">
        <CardHeader className="flex min-w-0 flex-row items-start justify-between space-y-0 overflow-visible px-3 pb-2 pt-3">
          ...
          >
            <div className="relative shrink-0 p-1">
              ...
              {isAuthorVerified && (
                <span className="absolute bottom-0 right-0 inline-flex h-[18px] w-[18px] items-center justify-center rounded-full bg-slate-950 ring-2 ring-background">
        ...
        <CardContent className="min-w-0 space-y-3 px-3 pb-3">
          {isRepost && (
            <div className="inline-flex max-w-full items-center gap-1.5 rounded-full bg-muted/50 px-2.5 py-1 text-[11px] font-semibold text-muted-foreground">
              <Repeat2 className="h-3.5 w-3.5" />
              <span className="truncate">
                Reposted{originalAuthorName ? ` from ${originalAuthorName}` : ''}
              </span>
            </div>
          )}
                ...
                <div className="relative flex h-full w-full flex-col bg-black text-white">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 z-30 border-t border-white/10 bg-black/80 px-5 py-4">
                    <div className="mx-auto grid max-w-md grid-cols-5 gap-2 text-white">
                      <button
                        type="button"
                        onClick={() => handleReactionSelect(activeReaction || 'like')}
                        className={viewerActionButton}
                      >
                        <ReactionButtonIcon reaction={activeReaction} active={hasReacted} />
          ...
          <div className="space-y-2 pt-0.5">
            <div className="flex min-w-0 items-center justify-between gap-1 border-y border-border/40 py-1">
              <DropdownMenu modal={false}>
                <DropdownMenuTrigger asChild>
                  <button
                    type="button"
                    aria-label="React"
                    className={`${actionButton} ${hasReacted ? activeReactionMeta.className : ''}`}
                  >
                    <ReactionButtonIcon reaction={activeReaction} active={hasReacted} />
                ...
                <DropdownMenuContent
                  align="start"
                  className="flex gap-1 rounded-full border border-border/70 bg-popover/95 px-2 py-2 shadow-xl backdrop-blur-xl"
                >
                  {REACTION_OPTIONS.map((item) => {
                    const Icon = item.icon;
                    const isActive = activeReaction === item.type || (item.type === 'wow' && activeReaction === 'smile');

                    return (
                      <DropdownMenuItem
                        key={item.type}
                        onClick={() => handleReactionSelect(item.type)}
                        className={`rounded-full px-2 py-2 ${isActive ? 'bg-muted' : ''}`}
                        title={item.label}
                      >
              ...
              <button
                ...
                onClick={handleRepost}
                disabled={repostBusy}
                title="Repost to your timeline and feed"
              >
                      ...
                      <div className="mt-1 text-sm leading-relaxed text-foreground">
                        {isVoice && comment.voiceUrl ? (
                          ...
                          <div className="whitespace-pre-wrap break-words">
                            {renderLinkedText(commentTextSafe)}
                          </div>
                        )}
                      </div>
